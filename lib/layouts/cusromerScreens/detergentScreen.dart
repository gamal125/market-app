...
class DetergentScreen extends StatefulWidget {
  const DetergentScreen({Key? key}) : super(key: key);

  @override
  State<DetergentScreen> createState() => _DetergentScreenState();
}

class _DetergentScreenState extends State<DetergentScreen> {
  @override
  Widget build(BuildContext context) {
    var category_name='منظفات';
    ...
    return BlocConsumer<MainCubit, MainStates>(
      ...
      listener: (context, state) {
        ...
        if(state is GetCategoriesSuccessStates||state is SendtoWhatsSuccessState) {
          c.ItemsPro.clear();
          c.getAllItems(category_name: category_name,Itemm:c.DetergentItems);
                  ...
                  child: Scrollbar(
                    ...
                    child: ListView.separated(
                      ...
                      itemBuilder: (context, index) => CategoriesItem(
                          c.DetergentItems[index], category_name,context),
                      separatorBuilder: (context, index) => SizedBox(
                        width: 10.0,
                      ),
                      itemCount: c.DetergentItems.length,
                    ),
                  ),
                ),
