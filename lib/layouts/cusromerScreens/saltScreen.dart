...
class SaltScreen extends StatefulWidget {
  const SaltScreen({Key? key}) : super(key: key);

  @override
  State<SaltScreen> createState() => _SaltScreenState();
}

class _SaltScreenState extends State<SaltScreen> {
  var category_name='توابل';
  @override
    ...
    return BlocConsumer<MainCubit, MainStates>(
      ...
      listener: (context, state) {
        ...
        if(state is GetCategoriesSuccessStates||state is SendtoWhatsSuccessState) {
          c.ItemsPro.clear();
          c.getAllItems(category_name: category_name,Itemm:c.SaltItems);
                  ...
                  child: Scrollbar(
                    ...
                    child: ListView.separated(
                      ...
                      itemBuilder: (context, index) => CategoriesItem(
                          c.SaltItems[index], category_name,context),
                      separatorBuilder: (context, index) => SizedBox(
                        width: 10.0,
                      ),
                      itemCount: c.SaltItems.length,
                    ),
                  ),
                ),
