...
class adminMilkScreen extends StatelessWidget {
  const adminMilkScreen({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {

    var c=MainCubit.get(context);
    var category_name='البان';
    ...
    return BlocConsumer<MainCubit, MainStates>(
      ...
      listener: (context, state) {
        if(state is GetCategoriesSuccessStates|| state is deleteProductsSuccessStates ||state is UpdateProductSuccessStates) {
          c.ItemsPro.clear();
          c.getAllItems(category_name: category_name,Itemm:c.MilkItems);}
      },
                  ...
                  child: Scrollbar(
                    ...
                    child: ListView.separated(
                      ...
                      itemBuilder: (context, index) => CategoriesItem(
                          c.MilkItems[index],category_name, context),
                      separatorBuilder: (context, index) => SizedBox(
                        width: 10.0,
                      ),
                      itemCount: c.MilkItems.length,
                    ),
                  ),
                ),
                Column(
