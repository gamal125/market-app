...
class adminBreadScreen extends StatelessWidget {
  const adminBreadScreen({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {

    var c=MainCubit.get(context);
 var category_name='خبز';
    ...
    return BlocConsumer<MainCubit, MainStates>(
      ...
      listener: (context, state) {
        if(state is GetCategoriesSuccessStates|| state is deleteProductsSuccessStates ||state is UpdateProductSuccessStates) {
          c.ItemsPro.clear();
          c.getAllItems(category_name: category_name,Itemm:c.BreadItems);}
      },
                  ...
                  child: Scrollbar(
                    ...
                    child: ListView.separated(
                      ...
                      itemBuilder: (context, index) => CategoriesItem(
                          c.BreadItems[index],category_name, context),
                      separatorBuilder: (context, index) => SizedBox(
                        width: 10.0,
                      ),
                      itemCount: c.BreadItems.length,
                    ),
                  ),
                ),
                Column(
